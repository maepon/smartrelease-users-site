...
args: '-o ConnectTimeout=5 --size-only'
```
通常、ファイルのタイムスタンプ比較などで同期判定を行いますが、SmartRelease U のような一部のサーバー環境ではタイムスタンプが正しく維持されないことがあります。`--size-only` を指定することで、ファイルサイズに変化がないファイルをスキップし、変更のあった最小限のファイルだけを高速に差分アップロードすることができます。この方法では、数文字の文言修正や、設定ファイルのフラグ反転（true → false）など、文字数が変わらない修正が反映されないリスクがありますが、今回はこのリスクについては許容する前提で設定しています。
